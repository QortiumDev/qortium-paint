...
const DEFAULT_NAME = 'Paint';
const DEFAULT_IDENTIFIER = 'Paint';
const DEFAULT_TITLE = 'Paint';
const DEFAULT_DESCRIPTION = 'Draw, edit, and join images; save locally or publish to Qortium QDN.';
...
const nodeApiUrl = (process.env.QORTIUM_PAINT_NODE_API_URL ?? DEFAULT_NODE_API_URL).replace(/\/+$/, '');
const publishName = process.env.QORTIUM_PAINT_QDN_NAME ?? DEFAULT_NAME;
const identifier = process.env.QORTIUM_PAINT_QDN_IDENTIFIER ?? DEFAULT_IDENTIFIER;
const publishTitle = process.env.QORTIUM_PAINT_QDN_TITLE ?? DEFAULT_TITLE;
const service = process.env.QORTIUM_PAINT_QDN_SERVICE ?? 'APP';
const distPath = path.resolve(repoRoot, process.env.QORTIUM_PAINT_DIST_PATH ?? 'dist');
const apiKeyPath = expandHomePath(
  process.env.QORTIUM_PAINT_NODE_API_KEY_PATH ?? '~/.config/qortium-core/runtime/apikey.txt',
);
const previewAccountsPath = expandHomePath(
  process.env.QORTIUM_PAINT_PREVIEW_ACCOUNTS_PATH ??
...
function getApiKeySource() {
  const explicitApiKey = process.env.QORTIUM_PAINT_NODE_API_KEY?.trim();
  ...
  if (process.env.QORTIUM_PAINT_NODE_API_KEY_PATH?.trim()) {
    return {
...
if (
  ...
  process.env.QORTIUM_PAINT_ALLOW_REMOTE_SIGN !== '1'
) {
  throw new Error(
    `Refusing to send the account private key to non-loopback HTTP node ${nodeApiUrl}. ` +
      'Use a local node or an https:// URL, or set QORTIUM_PAINT_ALLOW_REMOTE_SIGN=1 to override.',
  );
}
